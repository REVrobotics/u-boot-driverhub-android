...
static int rk817_bat_is_exist(struct udevice *dev){
	...
	/* Read the data of adc channel 0 to determine whether the battery exists. */
	unsigned int adcval;
	adc_channel_single_shot("saradc", 0, &adcval);
	debug("adc value is %d.\n", adcval);

	if(adcval){
        return 1;  // battery exists.
    }
